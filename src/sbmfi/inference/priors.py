...
from sbmfi.core.polytopia import LabellingPolytope, FluxCoordinateMapper, \
    ...
    compute_polytope_halfspaces, V_representation

from collections import OrderedDict
#  distance from polytope things, not (yet) necessary
...
class _BasePrior(Distribution):
    def __init__(
        ...
        self._ic = cache_size  # current index in the cashe
    ...
    def _fill_caches(self, n=20000, close_pool=True):
    ...
    def sample_dataframes(self, n=10000, close_pool=True, **kwargs) -> (pd.DataFrame, pd.DataFrame):
        self._fill_caches(n=n, close_pool=close_pool, **kwargs)
...
class _FluxPrior(_BasePrior):
    ...
    def _run_tasks(self, tasks, fn=coordinate_hit_and_run_cpp, break_i=-1, close_pool=True):
        if self._num_processes > 0:
...
class UniFluxPrior(_FluxPrior):
    def _fill_caches(self, n=20000, close_pool=True):
        ...
        result = coordinate_hit_and_run_cpp(
            self._fcm._sampler, initial_points=self._basis_points, n=n,
            new_basis_points=True, return_basis_samples=True,
        )
        self._basis_points = result['new_basis_points']
        theta, fluxes = self._fcm.append_xch_flux_samples(result['fluxes'], result['basis_samples'])
        self._theta_cache = torch.from_numpy(theta)
        self._flux_cache = torch.from_numpy(fluxes)
...
class _RatioSupport(Constraint):
    def __init__(
        ...
        polytope = fcm._sampler._F_simp
        if project:
            ratio_reactions = []
            for ratio_id, num_den in ratio_repo.items():
                ratio_reactions.append(*num_den['denominator'].keys())
        ...
        locations = [polytope.S.columns.get_loc(reaction.id) for reaction in polytope.objective.keys()]
        c = np.zeros(n_vars)
        ...
        self._numarr = np.zeros((self._nrat, polytope.S.shape[1]), dtype=np.double)
        self._denarr = np.zeros((self._nrat, polytope.S.shape[1]), dtype=np.double)
        for i, (ratio_id, vals) in enumerate(ratio_repo.items()):
            num_idxs = np.array([polytope.S.columns.get_loc(key) for key in vals['numerator'].keys()])
            ...
            den_idxs = np.array([polytope.S.columns.get_loc(key) for key in conden.keys()])
        ...
        den_lhs_df = pd.DataFrame(-denominator_lhs.value, index=index + '_den', columns=polytope.S.columns)
        ...
        lhs_df = pd.DataFrame(self._lhs.value, index=index, columns=polytope.S.columns)
        ...
        self._reaction_ids = polytope.S.columns
        self._constraint_ids = index

    def construct_polytope_constraints(self, ratio_sample: np.array) -> np.array:
        if len(ratio_sample.shape) == 1:
            ratio_sample = ratio_sample[None, :]  # means that it is now 2D

        vape = ratio_sample.shape
        viewlue = ratio_sample.view(vape[:-1].numel(), vape[-1])
    ...
    def check(self, value: torch.Tensor) -> torch.Tensor:
        ...
        nv = viewlue.shape[0]
        self._accepted = torch.zeros((nv, ), dtype=np.bool_)
        self._optima = np.zeros((nv, ), dtype=np.double)

        for i in range(viewlue.shape[0]):
            ratio_sample = viewlue[i, :]
            self._lhs.value = self.construct_polytope_constraints(ratio_sample=ratio_sample)[0, ...].numpy()
            try:
...
class RatioPrior(_BasePrior):
    ...
    def ratio_variability_analysis(fcm: FluxCoordinateMapper, model: RatioMixin, min_denom_sum = 0.0, positive_numerator=True) -> pd.DataFrame:
        ...
        :return:
        """
        thermo_pol = extract_labelling_polytope(model, coordinates='thermo', include_cofactors=True)
    ...
    def _fill_caches_rejection(
        ...
        #     input_q.put((task_i, ratio_samples), block=True)
        #     task_i += 1
        # result = []
        # _worker(input_q, output_q, self._support, self._n_flux)
        # _listener(output_q, n*n_flux, result)
        # return
        ...
        #     proc = mp.Process(
        #         target=_ratio_worker, name=f'worker_{i}', args=[input_q, output_q, self._support], daemon=True
        #     )
        #     proc.start()
        ...
        if self._num_processes == 0:
            ...
            listener = threading.Thread(
                # NB the factor 1.1 is to avoid problems with polytope that pass the check but that fail the rounding step
                target=_ratio_listener, name=f'listener', args=[self._output_q, m, result], daemon=True
            )
            listener.start()
            task_i = 0
            while listener.is_alive():
                # sample ratios from the ratio distribution; many will be rejected, so we sample more than n
                ratio_samples = ratio_dist.sample((batch_size, ))
                self._input_q.put((task_i, ratio_samples), block=True)
                task_i += 1

            while not self._input_q.empty():
                # empty input queue so that the workers wont finish the whole thing unnecessarily
                self._input_q.get()

            # for proc in processes:
            #     # tell processes to quit
            #     input_q.put(0)
            i = 0
            while i < len(processes):
                # make sure that the output queue is not being filled anymore
                oput = output_q.get()
                if isinstance(oput, int) and (oput == 0):
                    i += 1
        ...
        if n_flux == 0:
            ...
            sampling_task_generator = sampling_tasks(self._support._ratio_pol, counts=n_flux, **kwargs)
            pool = mp.Pool(processes=self._num_processes)
            result = pool.starmap(_sample_polytope_uniformely_suppress_output, iterable=sampling_task_generator)
    ...
    def _fill_caches_usm(self, n=1000, n_flux=20):
...
class ProjectionPrior(_FluxPrior):
    ...
    def __init__(
        ...
        self._boundary_vsm = PolytopeSamplingModel(self._projection_pol)
    ...
    def _fill_caches(self, n=100, n_flux=10, rel_tol=0.01, break_i=-1, close_pool=True):
        boundary_result = coordinate_hit_and_run_cpp(
            self._boundary_vsm, n=n, initial_points=self._projection_basis_points,
