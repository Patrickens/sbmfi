...
def _create_coli_glc_dataset(
    ...
    prior = UniRoundedFleXchPrior(model=model, cache_size=n_train, num_processes=1, logit_xch_fluxes=True)
...
def _create_designs(from_scratch=True):
    ...
    prior = UniRoundedFleXchPrior(model=model, cache_size=n, num_processes=1, logit_xch_fluxes=True)
    substrate_df = kwargs['substrate_df']
