...
from sbmfi.core.util import make_multidex, get_tensor, tensormul_T
...
class MDV_ObservationModel(object):
    ...
    def scaling(self):
        ...
        for ion_id, indices in self._ionindices.items():
            scaling[ion_id] = self._scaling[indices][0].detach().cpu().numpy()
    ...
    def _set_scaling(self, scaling: pd.Series, ionindices, transform_scaling=True):
        ...
        for ion_id, value in scaling.items():
            ...
            if indices is not None:
                value = get_tensor(values=np.array([value]), device=self._device)
                self._scaling[indices] = value
                mdv_indices = get_tensor(values=odf.loc[(odf['ion_id'] == ion_id), 'state_idx'].values, device=self._device)
    ...
    def check_x_meas(self, x_meas: pd.Series, atol=1e-3):
        ...
        x_meas = torch.atleast_2d(get_tensor(values=x_meas, device=self._device))
        if self._transformation is not None:
    ...
    def _set_natural_abundance_correction(self, isotope_threshold=1e-4, correction_threshold=0.001):
        ...
        return get_tensor(shape=(tot_obs, tot_obs), values=values, indices=indices, device=self._device)
    ...
    def _sum_square_diff(self, data, x_meas):
        ...
        if len(data.shape) < 3:
            data = data.unsqueeze(-2)  # adds the n_obs dimension for simulations with n_obs=0 in the previous step
        return (data - x_meas).pow(2).sum(-1)  # sum over n_data
    ...
    def rmse(self, data, x_meas):
        ...
        # rmse = n_samples
        return diff_2.mean(-1).sqrt()  # mean over n_obs

    def __call__(self, mdv, n_obs=3, pandalize=False, **kwargs):
        index = None
        if isinstance(mdv, pd.DataFrame):
            index = mdv.index
            mdv = get_tensor(values=mdv.loc[:, self.state_id].values, device=self._device)
        ...
        if pandalize:
            ...
            if len(result.shape) > 2:
                result = result.detach().cpu().numpy().transpose(1, 0, 2)
...
class MDV_LogRatioTransform():
        ...
        if transformation == 'ilr':
            if ilr_basis == 'helmert':
                ...
                for ion_id, df in observation_df.groupby('ion_id', sort=False):
                    ...
                    basis = get_tensor(values=helmert(df.shape[0], full=False), device=self._device)
                    k, l = basis.shape
                    self._ilr_basis[j: j + l, i: i + k] = basis.T
                    self._sumatrix[j: j + l, j: j + l]  = 1.0
                    i += k
                    j += l
                self._scaled_sumatrix = self._sumatrix.clone()
                self._meantrix = self._sumatrix / self._sumatrix.sum(0, keepdim=True)
    ...
    def _closure(self, mat, sumatrix=None):
        if sumatrix is None:
            sum = mat.sum(dim=-1, keepdim=True)
        else:
            sum = mat @ sumatrix
        return mat / sum

    def _clr_inv(self, clrs, sumatrix=None):
        expclrs = torch.exp(clrs)
        return self._closure(expclrs, sumatrix)

    def _clr(self, observations, meantrix=None):
        logobs = torch.log(observations)
        if meantrix is None:
            mean = logobs.mean(dim=-1, keepdim=True)
        else:
    ...
    def set_scaling(self, scaling):
        scaling = torch.atleast_2d(scaling)
...
class _BlockDiagGaussian(object):
    ...
    def __init__(self, observation_df: pd.DataFrame, device=None, batch_size: int = 1):
        self._device = device if device is not None else torch.device('cpu')
        self._observation_df = observation_df
        ...
        for denomi, ((model_id, ion_id, ion), df) in enumerate(
            ...
            self._ionindices[ion_id] = get_tensor(values=indices_feature, device=self._device)
        ...
        self._indices = get_tensor(values=sigma_indices, device=self._device)
        self._row = get_tensor(values=sigma_indices[:, 0], device=self._device)
        self._col = get_tensor(values=sigma_indices[:, 1], device=self._device)
        self._denom = get_tensor(values=sigma_indices[:, 2], device=self._device)
        self._mdv = get_tensor(values=sigma_indices[:, 3], device=self._device)
        ...
        self._diag = get_tensor(values=diagionals, device=self._device)
        self._uptri = get_tensor(values=offdiag_uptri, device=self._device)

        # these are used to distribute values into a vector of features
        self._numi = get_tensor(values=observation_df['state_idx'].values, device=self._device)
        self._denomi = self._denom[self._diag]

        denom_sum_indices = np.unique(sigma_indices[:, [2, 1]], axis=0)
        self._denom_sum = get_tensor(
    ...
    def sigma_1(self):
        if self._sigma_1 is None:
            self._sigma_1 = torch.linalg.pinv(self._sigma, rcond=1e-12, hermitian=False)
    ...
    def construct_sigma_x(observation_df: pd.DataFrame, diagonal_std: pd.Series = 0.1, corr=0.0):
        ...
        diagonal_std = diagonal_std.loc[observation_df.index]
        idx = _BlockDiagGaussian(observation_df=observation_df)
        nf = len(diagonal_std)
        ...
        if corr > 0.0:
            uptri_np = idx._uptri.numpy()
            indices_np = idx._indices.numpy()
            sigma[indices_np[uptri_np, 0], indices_np[uptri_np, 1]] = \
                np.prod(std[indices_np[uptri_np, :2]], axis=1) * corr
        sigma += sigma.T
    ...
    def set_sigma(self, sigma, verify=True):
        ...
        if verify:
            sigma = get_tensor(values=sigma, device=self._device)  # making sure we have the correct type
            ...
            for i in range(sigma.shape[0]):
                ...
                is_diagonal = torch.allclose(sigma, sigma.transpose(-2, -1), rtol=1e-10)  # sigma must be diagonal
        ...
        self._sigma = sigma
        self._chol = torch.linalg.cholesky(self._sigma)  # NB fails if not invertible!
        self._sigma_1 = None
    ...
    def compute_observations(self, s: pd.DataFrame, select=True, pandalize=False):
        ...
        if isinstance(s, pd.DataFrame):
            index = s.index
            s = s.values
        s = get_tensor(values=s, device=self._device)
        observations_num = s
        if select:
            observations_num = s[..., self._numi]
        observations_denom = tensormul_T(self._denom_sum, observations_num)
        observations_denom[observations_denom == 0.0] = 1.0
        observations = observations_num / observations_denom[..., self._denomi]
        if pandalize:
            observations = pd.DataFrame(observations.detach().cpu().numpy(), index=index, columns=self._observation_df.index)
        return observations

    def sample_sigma(self, shape=(1, )):
        noise = torch.randn(*shape, self._no, 1, dtype=torch.double, device=self._device)
        res = (self._chol.unsqueeze(1) @ noise).squeeze(-1)
        return res
...
class ClassicalObservationModel(MDV_ObservationModel, _BlockDiagGaussian):
    ...
    def __init__(
        ...
        _BlockDiagGaussian.__init__(self, observation_df=self._observation_df, device=self._device, batch_size=self._batch_size)
    ...
    def _initialize_J_xs(self):
        ...
        for i, (rowi, coli) in enumerate(zip(self._row, self._col)):
            ...
            if rowi == coli:
                ...
                if indices.size():
                    num_sum_indices.append([*zip(cycle([i]), indices.detach().cpu().numpy())])
                    num_sum_values.extend([1] * indices.shape[0])
            else:
                num_sum_indices.append([[i, coli.item()]])
                num_sum_values.append(-1)
        num_sum_indices = np.vstack(num_sum_indices)
        self._num_sum = get_tensor(
    ...
    def _random_selector_permutation(self, fullrank, rank):
        while True:
            permutation = torch.randperm(fullrank, device=self._device)[:rank]
            permtup = tuple(permutation.tolist())
            if permtup not in self._permutations:
                self._permutations[permtup] = 0.0  # store determinants for every permutation
                selector = self._selector.clone()
    ...
    def sigma_v(self, mdv, J_sv, rtol=1e-10, n_tries=500):
        ...
        for i in range(mdv.shape[0]):
            ...
            invertible = False
            U, S, V = torch.linalg.svd(FIM[i], full_matrices=True)
            fullrank = S.shape[0]
            ...
            while not invertible and not (j > n_tries):
                ...
                U, S, V = torch.linalg.svd(FIM_act, full_matrices=True)
                rank_act = S.shape[0]
            ...
            if invertible:
                ...
                sigma_v[i, mask] = (V.T @ torch.diag(1.0 / S) @ U.T).flatten()
                summary_v[i, 0] = rank / fullrank
                summary_v[i, 1] = torch.trace(sigma_v[i])
                summary_v[i, 2] = 1.0 / S.prod()  # |sigma_v| = 1.0 / |FIM|
    ...
    def sample_observations(self, mdv, n_obs=3, **kwargs):
        ...
        if (self._cmin is not None):
            noisy_observations = noisy_observations.clamp(min=self._cmin)
    ...
    def log_lik(self, x_meas, mu_o):
        ...
        x_meas = torch.atleast_2d(x_meas)  # shape = n_meas x n_mdv
        mu_o = torch.atleast_2d(mu_o)  # shape = batch x n_d
...
class TOF6546Alaa5minParameters(object):
    ...
    def _exp_decay(self, I, lambada, cv_0, cv_o):
        return torch.exp(I * lambada) * cv_0 + cv_o
    ...
    def CV(self, I):
        return self._exp_decay(I, **self._popt_cv).clamp(max=1.0)  # CLIP TO ONE ORDER OF MAGNITUDE!!
...
class LCMS_ObservationModel(MDV_ObservationModel, _BlockDiagGaussian):
    ...
    def construct_sigma(self, logI):
        ...
        sigma += sigma.transpose(-2, -1).clone()  # easiest way to make it symmetric
        return sigma
    ...
    def sample_observations(self, mdv, n_obs=3, atol=1e-10, **kwargs):
        ...
        if self._natcorr:
            observations_num = tensormul_T(self._natcorr, observations_num)

        if n_obs == 0:
            clip_observations_num = observations_num.clamp(min=self._cmin)
            return self.compute_observations(clip_observations_num, select=False)

        log10_observations_num = torch.log10(observations_num + atol)
        noisy_observations = log10_observations_num[:, None, :].repeat(1, n_obs, 1)
...
class BoundaryObservationModel(object):
    def __init__(
        ...
        if check_noise_support:
            ...
            self._A = get_tensor(values=self._boundary_pol.A.values, device=self._device)
            self._b = get_tensor(values=self._boundary_pol.b.values, device=self._device)[:, None]
...
class MVN_BoundaryObservationModel(BoundaryObservationModel):
    def __init__(
        ...
        self._sigma_o = get_tensor(values=sigma_o, device=self._device)
        self._sigma_o_1 = torch.linalg.pinv(self._sigma_o, rcond=1e-12, hermitian=False)
    ...
    def sample_observation(self, mu_bo, n_obs=1, **kwargs):
        ...
        for i in range(n):
            ...
            rounds = 0
            while j < n_obs:
                noise = torch.randn(n_obs * 5, len(self._bound_id), dtype=torch.double, device=self._device) @ self._sigma_o
                samples = mean + noise

                valid = (self._A @ samples.T <= self._b).all(0)
    ...
    def log_lik(self, bo_meas, mu_bo):
        mu_bo = torch.atleast_2d(mu_bo)  # shape = batch x n_bo
        bo_meas = torch.atleast_2d(bo_meas)  # shape = n_obs x n_bo
